...
<body>
    ...
    <div  class="w-screen overflow-auto py-4 px-4 bg-gray-100">
     ...
     <div class=" overflow-auto py-4 px-4 bg-white rounded">
       ...
       <form action="{{ route('loans.handleApproval') }}" method="post" class="grid grid-cols-2 gap-4">
        @csrf
        @method('post')
        ...
        <div>
            ...
            <input name="guarantor" disabled type="text" value="{{ $approval->firstName .' '. $approval->lastName }}" class="border border-gray-300 rounded py-2 px-4 w-full text-blue-900 focus:outline-none">
        </div>
        <div>
            <label for="">Amount to be given (UGX)<span class="text-red-400">*</span></label><br>

            <input name="givenAmount" type="text" value="{{ $approval->amountRequested }}" class="border border-gray-300 rounded py-2 px-4 w-full text-blue-900 focus:outline-none">
        </div>
        <div>
